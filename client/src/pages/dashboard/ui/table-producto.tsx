...
interface DataTableDetalleProps<T> {
  ...
  onEdit?: (row: T) => void;
  onDelete?: (row: T) => void;
  onView?: (row: T) => void; // Nueva prop para ver detalles
  onRowSelect?: (row: T) => void;
  searchColumn?: keyof T;
  placeholderSearch?: string;
  actions?: React.ReactNode;
  viewRoute?: string; // Ruta base para ver detalles (opcional)
}

export function DataTableDetalle<T extends { id: string | number }>({
  columns,
  data,
  pageIndex = 0,
  pageSize = 10,
  ...
  placeholderSearch,
  actions,
  viewRoute = "/productos", // Ruta por defecto
  ...
  // Para navegación (elige según tu framework)
  const navigate = useNavigate(); // React Router
  // const router = useRouter(); // Next.js

  const handleView = (row: T) => {
    if (onView) {
      onView(row); // Si se proporciona callback personalizado
    } else if (viewRoute) {
      // Navegación automática por ID
      navigate(`${viewRoute}/${row.id}`); // React Router
      // router.push(`${viewRoute}/${row.id}`); // Next.js
    }
  };
