...
const listBrands = async (usuarioId: number) => {
    try {
        const [rows]: any = await cn
            .promise()
            .query("CALL sp_marca(?, ?, ?, ?)", ["LISTAR_MARCA", null, null, usuarioId])

        return {
            status: 200,
            success: true,
            data: rows[0] // aquí viene el resultado del SELECT
        }
    } catch (error: any) {
        return {
            status: 500,
...
const createBrand = async (brand: Brands) => {
    ...
    try {
        const [rows]: any = await cn
            .promise()
            .query("CALL sp_marca(?, ?, ?, ?)", ["INSERTAR_MARCA", null, brand.nombre, brand.usuarioId])
...
const updateBrand = async (brand: Brands) => {
    ...
    try {
        const [rows]: any = await cn
            .promise()
            .query("CALL sp_marca(?, ?, ?, ?)", ["ACTUALIZAR_MARCA", brand.id, brand.nombre, brand.usuarioId])
...
const deleteBrand = async (id: number, usuarioId: number) => {
    ...
    try {
        const [rows]: any = await cn
            .promise()
            .query("CALL sp_marca(?, ?, ?, ?)", ["ELIMINAR_MARCA", id, null, usuarioId])
